...
async function updateFocusTotals() {
    ...
    try {
        ...
        const total = (data || []).reduce(function(s,r) { return s + (r.duration_seconds || 0); }, 0);
...
function updateFocusTotalsLocal() {
    ...
    const total = getFocusHistory()
        .filter(function(s) { return new Date(s.started_at).toDateString() === todayStr; })
...
async function loadFocusHistoryFromDB() {
    ...
    const weekSec = (weekRes.data || []).reduce(function(s,r) { return s + (r.duration_seconds||0); }, 0);
...
function renderFocusHistoryLocal() {
    ...
    const weekStart = new Date(); weekStart.setDate(weekStart.getDate()-6); weekStart.setHours(0,0,0,0);
    const weekSec = h.filter(function(s) { return new Date(s.started_at) >= weekStart; })
                     .reduce(function(s,r) { return s + (r.duration_seconds||0); }, 0);
    const el = document.getElementById('fp-stat-week');
    if (el) el.textContent = formatStudyDuration(weekSec);
}
