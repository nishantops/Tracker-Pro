...
function renderUsersTable(users, planCounts, hoursPerUser) {
    ...
    if (!users.length) { tbody.innerHTML = '<tr><td colspan="10" class="a-empty">No users found.</td></tr>'; return; }
    tbody.innerHTML = users.map(function(u) {
        ...
        return '<tr data-uid="' + u.user_id + '">'
            + '<td><input type="checkbox" class="user-select-cb" data-uid="' + u.user_id + '" onchange="adminUpdateBulkBtn()"' + (isAdmin ? ' disabled title="Cannot delete admin"' : '') + '></td>'
    ...
    var selAll = document.getElementById('select-all-users');
...
function adminToggleSelectAll(checked) {
    var cbs = document.querySelectorAll('.user-select-cb:not(:disabled)');
    cbs.forEach(function(cb) { cb.checked = checked; });
    adminUpdateBulkBtn();
}

function adminUpdateBulkBtn() {
    var selected = document.querySelectorAll('.user-select-cb:checked');
...
function _getSelectedUserIds() {
    var ids = [];
    document.querySelectorAll('.user-select-cb:checked').forEach(function(cb) {
...
async function adminBulkDelete() {
    var ids = _getSelectedUserIds();
    if (ids.length === 0) return;
    var confirmed = confirm('⚠️ PERMANENT ACTION\n\nDelete ' + ids.length + ' user(s) and ALL their data?\n\nThis cannot be undone!');
    if (!confirmed) return;
    var doubleConfirm = confirm('Are you absolutely sure? Type OK to proceed.\n\nDeleting: ' + ids.length + ' account(s)');
    if (!doubleConfirm) return;

    showAdminToast('Deleting ' + ids.length + ' account(s)...', 'warning');
    try {
        var res = await adminClient.rpc('admin_delete_users', { target_user_ids: ids });
        if (res.error) throw res.error;
        await adminAuditLog('bulk_delete_users', null, { count: ids.length, user_ids: ids });
        showAdminToast(ids.length + ' account(s) permanently deleted', 'success');
        // Remove from cache and refresh table
        _allUsersCache = _allUsersCache.filter(function(u) { return ids.indexOf(u.user_id) === -1; });
        loadUsersTable();
    } catch(e) {
...
async function adminResetPassword() {
    if (!adminCurrentUser) return;
    var u = adminCurrentUser;
    var pwInput = document.getElementById('um-new-password');
    var newPw = pwInput.value;
    ...
    var confirmed = confirm('Reset password for "' + (u.display_name || u.email) + '"?');
    if (!confirmed) return;

    try {
        var res = await adminClient.rpc('admin_reset_password', { target_user_id: u.user_id, new_password: newPw });
        if (res.error) throw res.error;
        await adminAuditLog('reset_password', u.user_id, { name: u.display_name });
